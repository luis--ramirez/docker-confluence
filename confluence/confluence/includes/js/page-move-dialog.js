...
define('confluence/page-move-dialog', [
    ...
    return function PageMoveDialog() {
        ...
        var MovePageDialog = function (options) {
            ...
            structure.addPanel(AJS.I18n.getText("move.page.dialog.history.title"), Templates.MovePage.historyPanel({pageTitle: AJS.Meta.get("page-title")}), "history-panel", "history-panel-id");
            structure.addPanel(AJS.I18n.getText("move.page.dialog.browse.title"), Templates.MovePage.browsePanel({pageTitle: AJS.Meta.get("page-title")}), "browse-panel", "browse-panel-id");
            ...
            currentBreadcrumbs.update({spaceKey: AJS.Meta.get('space-key'), title: newParentPage || originalParent}, controls);
        ...
        var MovePageParams = function (spaceKey, pageTitle, siblingId, siblingRelativePosition) {
            var params = {
                ...
                pageId: AJS.Meta.get('content-id') || AJS.Meta.get('page-id'),
                spaceKey: spaceKey
            };
        ...
        {
            $('#move-page-dialog').find('.button-spinner').each(function (i, val) {$(val).spin();});
        }

        function stopButtonSpinner()
        {
            $('#move-page-dialog').find('.button-spinner').each(function (i, val) {$(val).spinStop();});
        }
        ...
        AJS.bind("deferred.page-move.editor", function (e) {
            ...
            new MovePageDialog({
                ...
                moveHandler: function (dialog, newSpaceKey, newSpaceName, newParentPage, targetId, newPositionIndicator, setErrors) {
                    ...
                    var isSharedDraftsEnabled = AJS.Meta.get('shared-drafts');
                    if (isSharedDraftsEnabled) {
                        ...
                        AJS.safe.ajax({
                            ...
                            type: "GET",
                            dataType: "json",
                            data: new MovePageParams(newSpaceKey, newParentPage, targetId, newPositionIndicator),
                            error: function() {
                                stopButtonSpinner();
                                setErrors(AJS.I18n.getText("move.page.dialog.move.failed"));
                            },
                            success: function(data) {
                                stopButtonSpinner();
                                var errors = [].concat(data.validationErrors || []).concat(data.actionErrors || []).concat(data.errorMessage || []);
