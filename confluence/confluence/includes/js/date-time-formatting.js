define('confluence/date-time-formatting', [
    'ajs',
    'moment'
], function(
    AJS,
    ...
    var DateTimeFormatting = {
        ...
        friendlyFormatDateTime: function(date, now, timezoneOffsetMillis) {
            ...
            if (timeSinceDate < 0) {
                return DateTimeFormatting.formatDateTime(date, timezoneOffsetMillis);
                ...
                return AJS.I18n.getText("x.mins.ago", DateTimeFormatting.wholeMinutesBetween(date, now));
            } else if (timeSinceDate < ninetyMinutes) {
                return AJS.I18n.getText("date.friendly.about.one.hour.ago");
            } else if (timeSinceDate > fiveHours && DateTimeFormatting.isYesterdayRelativeTo(date, now, timezoneOffsetMillis)) {
                return AJS.I18n.getText("date.friendly.yesterday", DateTimeFormatting.formatTime(date, timezoneOffsetMillis));
            } else if (timeSinceDate < twentyFourHours) {
                return AJS.I18n.getText("date.friendly.about.x.hours.ago", DateTimeFormatting.roundedHoursBetween(date, now));
            }

            return DateTimeFormatting.formatDate(date, timezoneOffsetMillis);
        }
    };
...
require('confluence/module-exporter')
    .exportModuleAsGlobal('confluence/date-time-formatting', 'AJS.DateTimeFormatting');